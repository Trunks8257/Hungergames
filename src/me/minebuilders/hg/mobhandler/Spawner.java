...
public class Spawner implements Runnable{
	...
	private EntityType pickRandomMob(boolean isNight, int x) {
		if (isNight) {
			if (x < 5) return EntityType.ZOMBIE;
			if (x < 8) return EntityType.SKELETON;
		} else {
			if (x < 3) return EntityType.COW;
			if (x < 6) return EntityType.PIG;
			if (x < 7) return EntityType.CHICKEN;
			if (x < 8) return EntityType.SHEEP;
			if (x < 9) return EntityType.SPIDER;
		}
