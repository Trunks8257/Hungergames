...
public class GameListener implements Listener {
	...
	public void onAttack(EntityDamageEvent event) {
		...
		if (victim instanceof Player) {
			...
			if (pd != null) {
				...
				if (g.getStatus() != Status.RUNNING) event.setCancelled(true);
			}
		}
	}
	...
	public void onPlayerClickLobby(PlayerInteractEvent event) {
		...
		if (event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			if (p.getItemInHand().getType().equals(Material.STICK) && plugin.players.containsKey(p.getUniqueId())) {
				useTrackStick(p);
			}
		}
	}
	...
	public void blockPlace(BlockPlaceEvent event) {
		...
		if (HG.manager.isInRegion(b.getLocation())) {
			...
			} else if (p.hasPermission("hg.create") && Arrays.asList(Status.BEGINNING, Status.RUNNING).contains(HG.manager.getGame(b.getLocation()).getStatus()) ) {
				return;
			} else {
				event.setCancelled(true);
			}
